...
local cfg = module("vrp_doors","doors")
...
AddEventHandler('vrpdoorsystem:open',function(id)
	...
	if vRP.hasPermission(user_id,cfg.list[id].perm) or vRP.hasPermission(user_id,cfg.list[id].perm2) or vRP.hasPermission(user_id, config['PermissaoMaster']..".permissao") or vRP.hasPermission(user_id, config['PermissaoAdmin']..".permissao") then
