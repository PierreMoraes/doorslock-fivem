...
src.deleteVehicle = function(veh) -- Sua função de deletar veiculo
	TriggerEvent('deletarveiculo', veh)
end
...
src.allDispHouses = function() -- Blips do comando /home disp
    for k,v in pairs(propriedades) do
        if proprietarios[k] == nil or v.tipo == "apartamento" then
            if proprietarios[k] ~= nil then
        ...
        if cauth then
            if not in_interior then
                for k in pairs(propriedades) do
                    if propriedades[k].coords ~= nil then
...
Citizen.CreateThread(function()
    Wait(8000)
    nation_config = vSERVER.getNation()

    while nation_config do
        if cauth then
            local time = 1000
            local ped = PlayerPedId()
            local pedCoords = GetEntityCoords(ped)

            if length(nearestGaragem) > 0 then
                for k,v in pairs(nearestGaragem) do
                    local distance = #(pedCoords - vec3(nearestGaragem[k].garagem['garagem'].x,nearestGaragem[k].garagem['garagem'].y,nearestGaragem[k].garagem['garagem'].z))
                    if distance <= 5.0 then
                        time = 5

                        config.drawlable(k, vec3(nearestGaragem[k].garagem['garagem'].x,nearestGaragem[k].garagem['garagem'].y,nearestGaragem[k].garagem['garagem'].z), "garagem", false)

                        if IsControlJustReleased(1, 51) and segundos <= 0 and distance <= 2.0  then
                            segundos = 3
                            if nearestGaragem[k].tipo  == "apartamento" then
                                if vSERVER.checkAPGarage(k) then
                                    vSERVER.openGaragem(k)
                                else
                                    config.lang['apGaragem']()
                                end
                            else
                                if vSERVER.checkHouseGarage(k) then
                                    vSERVER.openGaragem(k)
                                else
                                    config.lang['notownerGaragem']()
                                end
                            end
                        end 
                    end
...
Citizen.CreateThread(function()
    ...
    while nation_config do 
        ...
        if cauth then
            if not in_interior then
                for k in pairs(propriedades) do
                    if propriedades[k].garagem and propriedades[k].garagem['garagem'] ~= nil then
                        local distance = #(pedCoords - vec3(propriedades[k].garagem['garagem'].x,propriedades[k].garagem['garagem'].y,propriedades[k].garagem['garagem'].z))
                        if distance < 10 then
                            nearestGaragem[k] = propriedades[k]
                        elseif nearestGaragem[k] then
                            nearestGaragem[k] = nil
                        end
                    end
                end
            end
